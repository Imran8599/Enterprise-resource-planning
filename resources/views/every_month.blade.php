...
<div class="card">
    <div class="card-header card-header-info">
      <h4 class="card-title">Every Month Details</h4>
    </div>
    <div class="card-body">
        <form action="{{url('everymonth-search')}}" method="POST">
          @csrf
          <div class="row">
        ...
        @php
            $i = 0;
        @endphp
        <div class="pt-0 {{Request::is('everymonth-search')?$i=1:'collapse'}}">
          @if ($i == 1)
                ...
                <table class="table table-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{App\Extracost::monthExpanse($date)}} <small>TK</small></td>
                    </tr>
                    <tr>
                      <td><strong>Import</strong></td>
                      <td>{{App\Imports::monthImport($date)}} <small>TK</small></td>
                    </tr>
                    <tr>
                      <td><strong>Salary</strong></td>
                      <td>{{App\Payroll::monthSalary($date2)}} <small>TK</small></td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div class="col-md-3">
                <table class="table table-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$product->model}}</td>
                      <td>{{App\SellItem::monthSellModel($date,$product->model)}}</td>
                      <td>{{App\SellItem::monthSellPrice($date,$product->model)}}</td>
                    </tr>
                    @endforeach
                    <tr>
                      <td><strong>Total:</strong></td>
                      <td><strong>{{App\Cashmemo::monthSellQuantity($date)}} <small>Pitches</small></strong></td>
                      <td><strong>{{App\Cashmemo::monthIncome($date)}} <small>TK</small></strong></td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <div class="col-md-3">
                <table class="table table-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{$product->model}}</td>
                      <td>{{App\Production::monthMadeModel($date,$product->model)}}</td>
                    </tr>
                    @endforeach
                    <tr>
                      <td><strong>Total:</strong></td>
                      <td><strong>{{App\Production::monthMadeTotal($date)}} <small>Pitches</small></strong></td>
                    </tr>
                  </tbody>
                </table>
              </div>
                ...
                <table class="table table-sm">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{App\ImportsUsed::monthImportsUsed($date, $importType->import_type)}}</td>
                    </tr>
                    @endforeach
                    <tr>
                      <td><strong>Total:</strong></td>
                      <td><strong>{{App\ImportsUsed::monthUsedTotal($date)}} <small>Bag</small></strong></td>
                    </tr>
                  </tbody>
                </table>
              </div>
